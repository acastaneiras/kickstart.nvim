...
require('lazy').setup({
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      -- Note: We manually specify Mason package names here instead of using vim.tbl_keys(servers)
      -- because LSP server names don't always match Mason package names
      local ensure_installed = vim.tbl_keys(servers or {})
      vim.list_extend(ensure_installed, {
        -- You can add other tools here that you want Mason to install
        'lua-language-server', -- Lua Language server (LSP: lua_ls)
        'stylua', -- Used to format Lua code
        'vtsls', -- TypeScript/JavaScript (LSP: vtsls)
        'tailwindcss-language-server', -- Tailwind CSS (LSP: tailwindcss)
        'intelephense', -- PHP (LSP: intelephense)
        'emmet-language-server', -- Emmet (LSP: emmet_language_server)
      })
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        local disable_filetypes = { c = true, cpp = true, php = true, css = true, javascript = true, typescript = true }
      ...
      formatters_by_ft = (function()
        ...
        local config = { lua = { 'stylua' } }
        for _, filetype in ipairs(prettier_fts) do
          config[filetype] = { 'prettierd' }
        end
        return config
      end)(),
    },
...
if uname.release:lower():match 'microsoft' then require 'wsl' end
